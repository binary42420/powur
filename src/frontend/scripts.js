 ...
 */
 ...
 function loadParams()
    {
       ...
       for(const [ key , value ] of Object.entries(lsParams)) {
          gParams[ key ] = value;
       }
    }
 ...
 function saveParams()
    {
       localStorage.setItem(LOCAL_STORAGE_KEY , JSON.stringify(gParams));
    }
 ...
 async function analyzeSample(sample)
    {
       ...
       for(const processor of processors)
          {
             if ( processor.textContent !== processorName )
                {
                   continue;
                }
 ...
 function onOutputOptionSelected(event)
    {
       ...
       if ( item.onclick )
          {
             return;
          } // Event handled differently (e.g. a command menu item)
             ...
             for(const format of OUTPUT_FORMATS)
                {
                   if ( format !== value )
                      {
                         elementWithValue(format).checked = false;
                      }
                }
          }
       ...
       for(const location of locations)
          {
             ...
             if ( location === gParams.apiLocation )
                {
                   match = true;
                }
          }
       ...
       for(const [ name , proc_info ] of Object.entries(processors))
          {
             ...
             if ( gParams.processorName === name )
                {
                   match = option;
                }
          }
       if ( !match && 0 < eProcessor.children.length )
          {
             match = eProcessor.children[ 0 ];
          }
